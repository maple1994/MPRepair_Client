...
- (void)onPOISearchDone:(AMapPOISearchBaseRequest *)request response:(AMapPOISearchResponse *)response
{
    ...
    [self.tableView reloadData];
    [self tableView:self.tableView didSelectRowAtIndexPath:reloadIndexPath];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        AMapPOI *currentPoi = self.searchPoiArray[indexPath.row];
        self.currentSelectAddress = [NSString stringWithFormat:@"%@(%@)",currentPoi.name,currentPoi.address];
        self.currentlat = [NSString stringWithFormat:@"%f",currentPoi.location.latitude];
        self.currentlng = [NSString stringWithFormat:@"%f",currentPoi.location.longitude];
    ...
    self.currentSelectAddress = [NSString stringWithFormat:@"%@(%@)",self.selectedPoi.name,self.selectedPoi.address];
    self.currentlat = [NSString stringWithFormat:@"%f",self.selectedPoi.location.latitude];
    self.currentlng = [NSString stringWithFormat:@"%f",self.selectedPoi.location.longitude];
...
- (instancetype)initWithCoder:(NSCoder *)coder
{
    ...
    if (self) {
        ...
        self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, (SafeAreaHeight/2) -50) style:UITableViewStylePlain];
        self.tableView.delegate = self;
        self.tableView.dataSource = self;
