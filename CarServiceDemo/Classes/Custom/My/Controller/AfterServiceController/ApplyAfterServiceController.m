...
@interface ApplyAfterServiceController ()<UICollectionViewDelegate,UICollectionViewDataSource,SelectPhotoCollectionCellDelegate,UITextFieldDelegate>
...
@implementation ApplyAfterServiceController
...
- (void)loadAllView{
    self.title = @"售后服务";
    [self.applyButton setCornerRadius:4];
    self.textView.placeholder = @"请描述您申请售后的项目";
...
static ZLPhotoActionSheet * extracted(ApplyAfterServiceController *object) {
...
- (IBAction)applyButtonAction:(id)sender {
    ...
    if ([ValidateUtil isEmptyStr:self.textView.text]) {
        [MBProgressHUD showErrorOnView:self.view withMessage:@"请描述您申请售后的项目！"];
        return;
    }
    ...
    kWeakSelf(weakSelf)
    [[NetWorkingUtil netWorkingUtil] postDataWithPath:url parameters:parmas result:^(id obj, int status, NSString *msg) {
